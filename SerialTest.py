...
def process_wod(packets):
    ...
    with open('wod.csv', 'w', newline='') as csvfile:
        header = ['time', 'mode', 'bat_voltage', 'bat_current', 'bus_3v_current', 'bus_5v_current', 'temp_comm', 'temp_eps', 'temp_battery']
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for i in range(0, 32):
            time_column = [time + 60*i] # Increment by 1 minute for each iteration
